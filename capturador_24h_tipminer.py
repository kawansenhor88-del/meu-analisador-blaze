...
def salvar_rodada(rodada):
    ...
    reconexão, o PostgreSQL simplesmente ignora a duplicata.
    """
    conn = conectar_banco()
...
def converter_cor(valor):
    ...
    # Alguns eventos podem trazer o número diretamente.
    try:
        numero = int(valor)
...
def normalizar_rodada(payload):
    ...
    if resultado is None and color is None and numero is None:
        return None

    # Prioridade: cor explícita -> número -> tipo do evento.
    cor = None

    if color is not None:
        cor = converter_cor(color)

    if cor not in ("Vermelho", "Preto", "Branco") and numero is not None:
    ...
    while True:
        ...
        try:
            ...
            for linha in resposta.iter_lines(
                ...
                # requests pode devolver bytes em alguns cenários.
