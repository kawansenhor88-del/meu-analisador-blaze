...
def adicionar_rodada(payload):
    ...
    valor_cor = color if color is not None else resultado
...
def obter_historico():
    ...
    if not dados:
        raise RuntimeError("Ainda não recebi nenhuma rodada DOUBLE do TipMiner.")
...
def responder_usuario(message):
    ...
    try:
        ...
        print("OBTENDO HISTÓRICO DO POSTGRESQL...")
        dados_double = obter_historico()
        print("HISTÓRICO POSTGRESQL OBTIDO.")
        print("TAMANHO DO HISTÓRICO:", len(dados_double))
