...
def carregar_historico_fixo_tipminer():
    print("========================================")
    print("CARREGANDO 2.000 RODADAS DO TIPMINER")
    print("SEM SSE — BASE FIXA")
    print("========================================")
    ...
    if len(rodadas) < ANALYSIS_ROUNDS:
        raise RuntimeError(f"API retornou apenas {len(rodadas)} rodadas válidas; esperado: {ANALYSIS_ROUNDS}.")

    # Mantém exatamente as 2.000 recebidas, em ordem temporal antiga -> recente.
    ...
    print("TOTAL FIXO NO POSTGRESQL:", total)
    if total != ANALYSIS_ROUNDS:
        raise RuntimeError(f"Banco ficou com {total} rodadas; esperado: {ANALYSIS_ROUNDS}.")
    return total
...
def iniciar(message):
    ...
    bot.reply_to(
        message,
        "🤖 Bot online!\n\n"
        "Histórico fixo de 2.000 rodadas carregado.\n"
