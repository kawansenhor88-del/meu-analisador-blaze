...
def atualizar_historico_tipminer():
    """
    Consulta novamente o endpoint /history e substitui a base do PostgreSQL
    pelas 2.000 rodadas mais recentes retornadas pelo TipMiner.

    A API é a fonte da janela móvel: cada chamada pode trazer uma fotografia
    mais recente. O banco só é alterado depois que as 2.000 rodadas válidas
    foram confirmadas, para não apagar uma base válida em caso de erro.
    """
    print("========================================")
...
def carregar_historico_fixo_tipminer():
    """Carga inicial; depois, cada consulta ao bot atualiza a janela."""
    return atualizar_historico_tipminer()
...
def adicionar_rodada(payload):
    # Mantido apenas por compatibilidade: a base fixa NÃO recebe rodadas ao vivo.
    print("ℹ️ RODADA AO VIVO IGNORADA — histórico fixo de 2.000 não será alterado.")
    return False
...
def painel_callback(call):
    try:
        ...
        # Toda análise do painel consulta novamente a History, garantindo
        # que as 2.000 sejam as mais recentes disponíveis no TipMiner.
...
def responder_usuario(message):
    try:
        ...
        # Atualiza a janela de 2.000 antes de responder qualquer pergunta.
        atualizar_historico_tipminer()
...
if __name__ == "__main__":
    ...
    print("BASE ATUALIZADA: 2.000 RODADAS MAIS RECENTES")
