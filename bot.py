...
def analisar_50_sequencias_de_10():
    """Analisa as 50 ocorrências mais recentes de blocos de 10 cores iguais."""
    dados_desc = []
    offset = 0
    TAMANHO_BLOCO = 5000

    # Carrega blocos do banco em ordem mais recente -> mais antiga até ter
    # material suficiente para localizar as 50 ocorrências mais recentes.
    while len(dados_desc) < MAX_HISTORY:
        ...
        # A análise precisa estar em ordem cronológica.
        dados = list(reversed(dados_desc))
        teste = _encontrar_sequencias_de_10(dados, limite=51)
        if len(teste) >= 51:
            break
...
def analisar_sequencias_de_cores_iguais():
    ...
    """
    dados = obter_historico_banco(limite=MAX_HISTORY)
    if not dados:
...
def abrir_painel(message):
    try:
        ...
        bot.reply_to(
            ...
            f"📚 Rodadas armazenadas: {total}\n"
            f"💾 Limite: {MAX_HISTORY:,}\n\n"
...
# ==============================================================================
# TESTE DIRETO DA API HISTORY DO TIPMINER
# ==============================================================================
# Este comando é ISOLADO:
# - não consulta PostgreSQL;
# - não usa Gemini;
# - não usa polling do Telegram;
# - usa o mesmo webhook já configurado no bot;
# - consulta diretamente o endpoint /history.
#
# Comando no Telegram:
#   /teste200
#
# O Telegram recebe somente:
#   001. 🔴 3
#   002. ⚫ 12
#   003. ⚪ 0
#   ...
...
def _extrair_numero_history(rodada):
    """Tenta encontrar o número original de uma rodada do History."""
    if not isinstance(rodada, dict):
        try:
            return int(rodada)
        except Exception:
    ...
    # O SSE do TipMiner usa 'roll', então ele vem primeiro.
    for campo in (
        "roll",
        "number",
        "numero",
        ...
        valor = rodada.get(campo)

        # Evita interpretar textos como "DOUBLE", "DEFAULT" ou "LUCKY".
        try:
            numero = int(valor)
            if 0 <= numero <= 14:
                return numero
...
def _encontrar_lista_history(obj):
    """
    Procura recursivamente uma lista que pareça conter registros de rodadas.
    Isso evita depender de um único nome de campo da resposta JSON.
    """
    if isinstance(obj, list):
        if obj:
            amostra = obj[:10]
            quantidade_com_numero = sum(
                1 for item in amostra
                if _extrair_numero_history(item) is not None
            )
            if quantidade_com_numero > 0:
                return obj

        for item in obj:
            encontrada = _encontrar_lista_history(item)
            if encontrada is not None:
                return encontrada

    elif isinstance(obj, dict):
        # Primeiro tenta os nomes mais prováveis.
        for campo in (
            "data",
            "history",
            "rounds",
            ...
            if campo in obj:
                encontrada = _encontrar_lista_history(obj[campo])
                if encontrada is not None:
                    return encontrada
        ...
        for valor in obj.values():
            if isinstance(valor, (dict, list)):
                encontrada = _encontrar_lista_history(valor)
                if encontrada is not None:
                    return encontrada
...
def _enviar_blocos_telegram(chat_id, linhas):
    """Envia as linhas em blocos seguros para o limite do Telegram."""
    texto = ""
...
def teste200_history(message):
    try:
        bot.reply_to(
            message,
            "🔎 Consultando diretamente a API History do TipMiner...\n"
            "🚫 Banco: não\n"
            "🚫 Gemini: não"
        )
        ...
        print("========================================")
        print("TESTE DIRETO HISTORY TIPMINER")
        ...
        if resposta.status_code != 200:
            bot.send_message(
                message.chat.id,
                f"❌ API History respondeu HTTP {resposta.status_code}"
            )
            return
        ...
        rodadas = _encontrar_lista_history(dados)

        if not rodadas:
            print("NÃO FOI ENCONTRADA LISTA DE RODADAS.")
            print("RESPOSTA JSON:")
        ...
        # Para este teste, queremos exatamente os 200 registros retornados.
        # Se a API retornar mais, usamos somente os 200 primeiros do bloco.
        rodadas_teste = rodadas[:200]

        linhas = []

        for posicao, rodada in enumerate(rodadas_teste, start=1):
            numero = _extrair_numero_history(rodada)

            if numero is None:
                linhas.append(f"{posicao:03d}. ❓")
            else:
                linhas.append(
                    f"{posicao:03d}. {_emoji_cor_numero(numero)} {numero}"
                )

        print("TOTAL DE REGISTROS ENCONTRADOS:", len(rodadas))
        print("TOTAL USADO NO TESTE:", len(rodadas_teste))

        bot.send_message(
            message.chat.id,
            "✅ TESTE DIRETO DA API\n\n"
            f"HTTP: {resposta.status_code}\n"
            f"📊 Registros encontrados: {len(rodadas)}\n"
            f"📌 Enviando: {len(rodadas_teste)}\n\n"
            "Formato: posição. cor número"
        )

        _enviar_blocos_telegram(message.chat.id, linhas)
