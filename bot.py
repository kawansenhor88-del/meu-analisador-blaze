...
def _montar_linhas_estrategia_surfe(registros, estrategia):
    ...
    for item in registros:
        ...
        linha = (
            f"{numero}{espaco_rodada}{cor_real}{numero_real}{espaco_cor}"
...
def _montar_surfe_duas_colunas(registros):
    ...
    # Mantém a mensagem como texto normal.
    # A primeira coluna vira a referência: cada linha recebe apenas
    # o preenchimento necessário para terminar na mesma largura visual.
    ...
    largura_alvo = max((_largura_visual(linha) for linha in vermelho), default=0)
    ...
    for esquerda, direita in zip(vermelho, preto):
        diferenca = largura_alvo - _largura_visual(esquerda)

        # Pequenas diferenças são compensadas com espaços finos,
        # sem alterar o conteúdo da primeira coluna.
        quantidade_finos = max(0, round(diferenca / 0.20))
        preenchimento = "\u2009" * quantidade_finos

        linhas.append(esquerda + preenchimento + direita)
