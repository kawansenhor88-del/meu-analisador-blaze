...
def _montar_linhas_estrategia_surfe(registros, estrategia):
    """Monta as linhas de uma estratégia."""
    linhas = []
    ...
    for item in registros:
        ...
        espacos_numero_rodada = "\u2007" * 3 if numero < 10 else ""
        # Usa espaços não separáveis para o Telegram manter visualmente os 3 espaços.
        espacos_numero_real = "\u2007" * 3 if len(numero_real) == 1 else ""
        linha = (
            f"{numero}{espacos_numero_rodada}{cor_real}{numero_real}{espacos_numero_real}"
...
def _largura_visual_surfe(texto):
    """Calcula uma largura estável para o alinhamento no Telegram."""
    largura = 0
    for caractere in texto:
        if caractere in "🔴⚫⚪❌✅":
            largura += 2
        elif unicodedata.east_asian_width(caractere) in ("W", "F"):
            largura += 2
        else:
            largura += 1
    return largura
...
def _preencher_coluna_surfe(texto, largura):
    """Completa a coluna com espaços de controle de largura fixa."""
    faltam = max(0, largura - _largura_visual_surfe(texto))
    return texto + ("\u2007" * faltam)


def _montar_surfe_duas_colunas(registros):
    """Monta os dois SURF lado a lado com separador vertical fixo."""
    vermelho = _montar_linhas_estrategia_surfe(registros, "Vermelho")
    preto = _montar_linhas_estrategia_surfe(registros, "Preto")

    linhas = [
        _preencher_coluna_surfe("Surf 🔴", 27) + "│" + "  Surf ⚫",
        "",
    ]

    for esquerda, direita in zip(vermelho, preto):
        # A primeira coluna recebe preenchimento de controle até um ponto fixo.
        # Assim, a segunda coluna começa sempre exatamente após a linha vertical.
        esquerda_fixa = _preencher_coluna_surfe(esquerda, 27)
        linhas.append(esquerda_fixa + "│  " + direita)
