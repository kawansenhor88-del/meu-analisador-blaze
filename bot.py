...
def _montar_linhas_estrategia_surfe(registros, estrategia):
    ...
    for item in registros:
        ...
        # Um espaço padronizado depois do número da rodada (01, 02, 03...).
        # SOMENTE 01 a 08 recebem um ajuste fino extra para a direita.
        # A parte inteira usa U+3164 e cada 0.1 usa U+200A.
        ajuste_01_08 = ""
        if 1 <= numero <= 8:
...
def _montar_surfe_duas_colunas(registros):
    ...
    for esquerda, direita in zip(vermelho, preto):
        ...
        if esquerda.endswith("✅"):
            separador = criar_espaco(ESPACO_APOS_CHECK)
        elif "❌G" in esquerda:
            separador = criar_espaco(ESPACO_APOS_G)
        else:
            separador = criar_espaco(ESPACO_APOS_G)
