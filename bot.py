...
def analisar_50_sequencias_de_10():
    ...
    O banco pode conter até 100.000 rodadas, mas a análise usa somente as
    ocorrências necessárias para chegar às 50 mais recentes. Para cada gatilho
    de 10 iguais, as posições 11ª a 15ª são literalmente as cinco rodadas
    imediatamente seguintes na ordem temporal.
    """
    dados_desc = []
    offset = 0
    TAMANHO_BLOCO = 5000
    ...
    while len(dados_desc) < MAX_HISTORY:
        ...
        # Se já temos uma janela suficientemente antiga para encontrar 50
        # ocorrências, paramos. O resultado final sempre é limitado a 50.
        dados = list(reversed(dados_desc))
        teste = _encontrar_sequencias_de_10(dados, limite=50)
        if len(teste) >= 50:
            break
    ...
    # _encontrar... devolve em ordem antiga -> recente; queremos as 50 mais recentes.
    ...
    for _, cor, seq, seguintes in ocorrencias:
        ...
        # Se faltarem rodadas no fim do histórico, analisamos apenas as que existem.
    ...
    if disponiveis:
        ...
        resumo += ["", "🏆 MAIOR FREQUÊNCIA", f"➡️ {melhor}ª RODADA — {melhor_pct:.1f}%"]
