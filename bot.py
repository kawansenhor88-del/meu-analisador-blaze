...
def analisar_surfe():
    """Analisa o SURFE usando os 2.000 registros e prepara 100 para exibição."""
    dados_2000 = list(reversed(obter_historico_banco(limite=ANALYSIS_ROUNDS)))
    if not dados_2000:
        return None

    # Ponto inicial: Branco mais recente.
    indice_branco = None
    for i in range(len(dados_2000) - 1, -1, -1):
        if normalizar_cor_analise(dados_2000[i]) == "Branco":
            indice_branco = i
            break

    if indice_branco is None:
        return {"resumo": "❌ Não encontrei nenhum ⚪ Branco nos 2.000 registros.", "blocos": []}

    branco = dados_2000[indice_branco]
    data_branco, hora_branco = formatar_data_hora(branco.get("instant"), branco.get("tempo"))

    # Mantém o sentido combinado: parte do Branco mais recente e percorre as rodadas anteriores.
    caminho = list(reversed(dados_2000[:indice_branco]))
    ...
    for rodada in caminho:
        ...
        for nome, jogaria in (("Preto", jogaria_preto), ("Vermelho", jogaria_vermelho)):
            if saiu == "Branco":
                resultados[nome] = "⚪ BRANCO"
                continue
            if saiu == jogaria:
                stats[nome]["acertos"] += 1
                if gale[nome] > 0:
                    stats[nome]["gale_counts"][gale[nome]] = stats[nome]["gale_counts"].get(gale[nome], 0) + 1
                gale[nome] = 0
                resultados[nome] = "✅ ACERTO"
            else:
                stats[nome]["erros"] += 1
                gale[nome] += 1
                stats[nome]["maior_gale"] = max(stats[nome]["maior_gale"], gale[nome])
    ...
    exibidos = registros[:100]

    def coluna(item, estrategia):
        jogaria = item["jogaria_preto"] if estrategia == "Preto" else item["jogaria_vermelho"]
        resultado = item["resultado_preto"] if estrategia == "Preto" else item["resultado_vermelho"]
        return (
            f"{item['numero']}️⃣ Saiu: {emoji_cor(item['saiu'])} {item['saiu'].upper()}\n"
            f"   Jogaria: {emoji_cor(jogaria)} {jogaria.upper()}\n"
            f"   {resultado}"
        )

    # Duas colunas lado a lado, em fonte monoespaçada, para manter tudo reto.
    LARGURA_COLUNA = 31
    linhas = [
        "🏄 SURFE — 2 PRETOS       🏄 SURFE — 2 VERMELHOS",
        "",
        f"⚪ BRANCO → {hora_branco:<12} ⚪ BRANCO → {hora_branco}",
        "",
    ]
    for item in exibidos:
        esq = coluna(item, "Preto").split("\n")
        dir_ = coluna(item, "Vermelho").split("\n")
        linhas.append(f"{esq[0]:<{LARGURA_COLUNA}} {dir_[0]}")
        linhas.append(f"{esq[1]:<{LARGURA_COLUNA}} {dir_[1]}")
        linhas.append(f"{esq[2]:<{LARGURA_COLUNA}} {dir_[2]}")
        linhas.append("")

    def resumo_estrategia(nome, titulo):
        st = stats[nome]
        total = st["acertos"] + st["erros"]
        pct = (st["acertos"] / total * 100) if total else 0
        partes = [
            titulo,
        ...
        for n in sorted(st["gale_counts"]):
            partes.append(f"📊 Gale {n}: {st['gale_counts'][n]} vezes")
        return "\n".join(partes)

    resumo_final = "\n".join([
        "📊 RESULTADO GERAL — 2.000 RODADAS",
        "",
        f"⚪ Branco de início mais recente: {data_branco} às {hora_branco}",
        "",
        resumo_estrategia("Preto", "⚫ SURFE — 2 PRETOS"),
        "",
        resumo_estrategia("Vermelho", "🔴 SURFE — 2 VERMELHOS"),
        "",
    ...
    introducao = "\n".join([
        ...
        "",
        "📚 Serão analisadas as 100 rodadas mais recentes.",
        "",
        "📌 COMO FUNCIONA:",
        "",
        "Após o primeiro ⚪ BRANCO encontrado,",
        "começamos dois caminhos simultaneamente:",
        "",
        "⚫ SURFE 2 PRETOS:",
        "⚫⚫ → 🔴🔴 → ⚫⚫ → 🔴🔴...",
        "",
        ...
        "normalmente até completar as 100 rodadas.",
        "",
        "━━━━━━━━━━━━━━━━━━",
        "",
        "👇 Clique abaixo para visualizar o caminho completo:",
    ])

    return {"intro": introducao, "caminho": "\n".join(linhas), "resumo": resumo_final}
...
def painel_markup():
    ...
    markup.add(
        telebot.types.InlineKeyboardButton("🏄 SURFE", callback_data="surfe")
    )
    markup.add(
...
def iniciar(message):
    ...
    try:
        # /start usa os registros que já estão salvos no banco.
        # Não consulta a API aqui para responder imediatamente.
        total = contar_rodadas_banco()
    except Exception:
        total = "indisponível"
...
@bot.callback_query_handler(func=lambda call: call.data in ("surfe_ver", "surfe_ocultar"))
def surfe_callback(call):
    try:
        bot.answer_callback_query(call.id)
        chat_id = call.message.chat.id

        if call.data == "surfe_ocultar":
        ...
        analise = surfe_cache.get(chat_id) or analisar_surfe()
        if not analise or "caminho" not in analise:
            bot.send_message(chat_id, "❌ Não foi possível gerar o SURFE agora.")
            return

        mensagens = []
        texto = ""
        for bloco in analise["caminho"].split("\n\n"):
            candidato = (texto + "\n\n" + bloco).strip() if texto else bloco
            if len(candidato) > 3000:
                if texto:
        ...
        ids = surfe_mensagens_abertas.get(chat_id, [])
        for parte in mensagens:
            m = bot.send_message(chat_id, f"<pre>{parte}</pre>", parse_mode="HTML")
        ...
        m = bot.send_message(
            ...
            reply_markup=telebot.types.InlineKeyboardMarkup().add(
                telebot.types.InlineKeyboardButton("🔽 OCULTAR SURFE", callback_data="surfe_ocultar")
            )
        )
        ids.append(m.message_id)
        surfe_mensagens_abertas[chat_id] = ids
    except Exception as erro:
        traceback.print_exc()
        try:
...
def painel_callback(call):
    try:
        ...
        if call.data == "surfe":
            analise = analisar_surfe()
            chat_id = call.message.chat.id
            if not analise:
                bot.send_message(chat_id, "❌ Ainda não há rodadas suficientes para analisar o SURFE.")
                return
            if "blocos" in analise:
                bot.send_message(chat_id, analise["resumo"])
                return
            ...
            msg = bot.send_message(
                ...
                analise["intro"],
                reply_markup=telebot.types.InlineKeyboardMarkup().add(
                    telebot.types.InlineKeyboardButton("👁️ VER SURFE", callback_data="surfe_ver")
                )
            )
            surfe_mensagens_abertas[chat_id] = [msg.message_id]
            surfe_cache[chat_id] = analise
            return
