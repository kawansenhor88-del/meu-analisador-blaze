...
def _montar_linhas_estrategia_surfe(registros, estrategia):
    """Monta uma estratégia por vez, mostrando número real, cor jogada e Gale."""
    ...
    for item in registros:
        ...
        # Mostra o número real que saiu junto da cor. Branco é neutro/ignorado.
        numero_real = item.get("numero_real", "?")
        if item["saiu"] == "Branco":
            linha_resultado = "⚪0  - " + emoji_cor(jogaria)
        else:
            if resultado.startswith("❌ GALE"):
                ...
                marcador = "✅"
            linha_resultado = (
                f"{emoji_cor(item['saiu'])}{str(numero_real):<2} - "
                f"{emoji_cor(jogaria)} {marcador}"
            )

        linhas.append(f"{int(item['numero']):>2}  {linha_resultado}")
...
def analisar_surfe_a_partir_do_branco(indice_branco, limite=None):
    """Analisa o SURF a partir do Branco, usando as rodadas imediatamente anteriores em ordem mais recente -> mais antiga."""
    dados = list(reversed(obter_historico_banco(limite=ANALYSIS_ROUNDS)))
    ...
    # anterior a ele (a mais recente em relação ao Branco) e segue para trás
    # no histórico (ex.: 11 -> 14 -> ...).
    caminho = list(reversed(dados[:indice_branco]))
...
def montar_resultado_surfe_50(analise):
    """Monta as 50 rodadas e o resumo em duas mensagens separadas."""
    ...
    return resultado_rodadas, resultado_resumo
...
def surfe_ver_callback(call):
    try:
        ...
        resultado_rodadas, resultado_resumo = montar_resultado_surfe_50(analise)

        # Envia as rodadas em uma mensagem e, logo depois, o resumo em uma nova mensagem.
        # Mensagens normais do Telegram, sem <pre>, para não criar o painel cinza.
        for resultado in (resultado_rodadas, resultado_resumo):
