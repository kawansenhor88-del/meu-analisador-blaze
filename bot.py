...
def _montar_surfe_duas_colunas(registros):
    ...
    for esquerda, direita in zip(vermelho, preto):
        # A segunda coluna usa exatamente a mesma montagem visual da primeira.
        # O espaço entre as colunas segue o marcador da primeira:
        # 2 espaços de controle após Gale e 4 após X/OK.
        separador = "\u2007" * (2 if "❌G" in esquerda else 4)
        # Pequeno ajuste: empurra somente o ❌/Gale da segunda coluna
        # para a direita, sem alterar a primeira coluna nem o ✅.
        if "❌G" in direita:
            direita = direita.replace("❌G", "\u2007❌G", 1)
