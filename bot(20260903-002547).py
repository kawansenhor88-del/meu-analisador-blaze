...
def analisar_atraso_do_branco():
    """Analisa todos os intervalos entre brancos nos 2.000 registros.

    Retorna o resumo separado dos intervalos para que o Telegram possa
    enviar cada intervalo como um bloco indivisível, no máximo 31 por mensagem.
    """
    dados = obter_historico_banco(limite=ANALYSIS_ROUNDS)
    if not dados:
        return "❌ Ainda não há rodadas suficientes no histórico fixo de 2.000."
    ...
    # Cada intervalo é criado como uma unidade completa. O callback abaixo
    # agrupa exatamente 31 unidades por mensagem, sem cortar texto no meio.
    blocos = []
...
def dividir_texto_telegram(texto, limite=3900):
    """Divide textos sem cortar uma linha no meio.

    O Telegram permite até 4096 caracteres por mensagem. Mantemos uma
    margem de segurança e preferimos quebrar sempre entre linhas, para
    não acontecer de uma data/horário ou intervalo ficar partido entre
    duas mensagens. Se uma linha isolada ultrapassar o limite, ela é
    dividida apenas nesse caso.
    """
    linhas = str(texto).splitlines(keepends=True)
    partes = []
    atual = ""
    ...
    for linha in linhas:
        if len(linha) > limite:
            if atual:
                partes.append(atual)
                atual = ""
            for inicio in range(0, len(linha), limite):
                partes.append(linha[inicio:inicio + limite])
            continue

        if atual and len(atual) + len(linha) > limite:
            partes.append(atual)
            atual = ""
...
@bot.callback_query_handler(func=lambda call: call.data in ("seq10", "seqcores", "branco_atraso", "ult50", "total", "ultima"))
    ...
    try:
        ...
        if call.data == "branco_atraso":
            ...
            if isinstance(resultado, list):
                resumo, blocos = resultado
                chat_id = call.message.chat.id

                # ABA 1: resumo completo.
                bot.send_message(chat_id, resumo)

                # ABA 2 em diante: intervalos encontrados.
                # São sempre 31 intervalos completos por mensagem.
                for inicio in range(0, len(blocos), 31):
                    grupo = blocos[inicio:inicio + 31]
                    numero_primeiro = inicio + 1
                    numero_ultimo = inicio + len(grupo)
                    ...
                    for indice, bloco in enumerate(grupo):
                        numero_atual = numero_primeiro + indice
                        partes.append(bloco)
                        if numero_atual != numero_ultimo:
                            partes.extend(["", "━━━━━━━━━━━━━━━━━━", ""])
