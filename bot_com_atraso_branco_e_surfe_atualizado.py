...
def _estatisticas_surfe(caminho):
    ...
    for rodada in caminho:
        ...
        for nome, jogaria in (("Preto", jogaria_preto), ("Vermelho", jogaria_vermelho)):
            # Branco não interrompe, não reinicia e não conta como acerto/erro.
            if saiu == "Branco":
                resultados[nome] = "⚪ BRANCO — IGNORADO"
                continue
...
def _montar_caminho_surfe(registros):
    """Formata o caminho das rodadas em duas colunas, 2 Pretos x 2 Vermelhos."""
...
def analisar_surfe_inicial():
    ...
    introducao = "\n".join([
        "🏄 SURF — ANÁLISE ESTATÍSTICA",
        "",
        "📌 Escolha um ⚪ Branco para definir o ponto de início do Surf.",
        "",
        "📊 Ao clicar em um Branco, serão analisadas as próximas rodadas",
        "em ordem cronológica, do horário mais antigo para o mais recente.",
        "",
        "⚫ SURFE 2 PRETOS:",
        "⚫⚫ → 🔴🔴 → ⚫⚫ → 🔴🔴...",
        "",
        ...
        "🔴🔴 → ⚫⚫ → 🔴🔴 → ⚫⚫...",
        "",
        "⚪ Os Brancos seguintes não interrompem nem reiniciam o caminho.",
        "",
        "📚 Primeiro serão mostradas até 50 rodadas a partir do Branco escolhido.",
        "Depois, o botão CONTROLE GERAL poderá analisar até a rodada mais recente.",
    ])
    return {"intro": introducao, "brancos": brancos}


def analisar_surfe_a_partir_do_branco(indice_branco, limite=None):
    """Analisa o SURF a partir de um Branco escolhido, sempre antigo -> recente."""
    dados = list(reversed(obter_historico_banco(limite=ANALYSIS_ROUNDS)))
    ...
    # O Branco escolhido é apenas o gatilho. As rodadas analisadas começam depois dele.
...
def montar_resultado_surfe_50(analise):
    """Resultado das primeiras 50 rodadas após o Branco escolhido."""
    registros = analise["registros"]
    texto = [
        "🏄 SURF — PRIMEIRAS 50 RODADAS",
        "",
        f"⚪ Branco inicial: {analise['data_branco']} às {analise['hora_branco']}",
        f"📚 Rodadas analisadas: {len(registros)}",
        "",
        _montar_caminho_surfe(registros),
    ]
    return "\n".join(texto)
...
def surfe_branco_callback(call):
    try:
        ...
        if not analise:
            bot.send_message(
                chat_id,
                "❌ Não foi possível localizar o Branco escolhido no histórico."
            )
            return

        # Guarda somente o ponto inicial escolhido para o CONTROLE GERAL.
        surfe_cache[chat_id] = {"branco_index": indice}

        resultado = montar_resultado_surfe_50(analise)

        ids = surfe_mensagens_abertas.setdefault(chat_id, [])
        partes = []
        texto = ""
        ...
        m = bot.send_message(
            chat_id,
            "👇 Depois das primeiras 50 rodadas, você pode conferir o histórico completo:",
            reply_markup=markup
        )
        ids.append(m.message_id)
