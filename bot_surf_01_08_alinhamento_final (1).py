...
def _montar_linhas_estrategia_surfe(registros, estrategia):
    ...
    for item in registros:
        ...
        ajuste_01_08 = ""
        if 1 <= numero <= 8:
            valor_ajuste = max(0.0, float(AJUSTE_RODADAS_01_08))
        ...
        linha = (
            f"{ajuste_01_08}{numero_formatado}{espaco_rodada}{cor_real}{numero_real}{espaco_cor}"
...
def _montar_surfe_duas_colunas(registros):
    ...
    for esquerda, direita in zip(vermelho, preto):
        ...
        numero_linha = int(esquerda.lstrip("\u3164\u200A")[:2])
        if 1 <= numero_linha <= 8:
            valor_separador = max(0.0, valor_separador - AJUSTE_RODADAS_01_08)
