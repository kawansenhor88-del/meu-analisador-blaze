...
def _montar_linhas_estrategia_surfe(registros, estrategia):
    ...
    for item in registros:
        ...
        # Teste de alinhamento: compensa somente a largura visual menor do ⚫.
        # Não altera 🔴 nem a lógica da estratégia.
        ajuste_preto_real = "\u2009" if cor_real == "⚫" else ""
        ajuste_preto_jogada = "\u2009" if cor_jogada == "⚫" else ""
        ...
        espaco_rodada = "\u2006" * (2 if numero < 10 else 1)
        espaco_cor = "\u2007" if len(numero_real) == 1 else ""

        linha = (
            f"{numero}{espaco_rodada}{cor_real}{ajuste_preto_real}{numero_real}{espaco_cor}"
            f"-{cor_jogada}{ajuste_preto_jogada}{marcador}"
        )
        linhas.append(linha)
...
def _preencher_visual_surfe(texto, largura_alvo):
    """Completa uma linha até a largura visual alvo usando espaço de figura."""
    faltam = max(0, largura_alvo - _largura_visual_surfe(texto))
    return texto + ("\u2007" * faltam)


def _montar_surfe_duas_colunas(registros):
    """Monta as duas colunas do SURF mantendo a segunda no mesmo ponto visual."""
    vermelho = _montar_linhas_estrategia_surfe(registros, "Vermelho")
    preto = _montar_linhas_estrategia_surfe(registros, "Preto")

    # A segunda coluna não pode depender de a primeira terminar em ✅ ou ❌G1/G2.
    # Primeiro igualamos a largura visual da primeira coluna em todas as linhas.
    largura_primeira = max(
        (_largura_visual_surfe(linha) for linha in vermelho),
        default=0,
    )
    ...
    for esquerda, direita in zip(vermelho, preto):
        esquerda_ajustada = _preencher_visual_surfe(esquerda, largura_primeira)
        # Pequeno espaço fixo entre as colunas; o alinhamento vem da primeira coluna.
        linhas.append(esquerda_ajustada + ("\u2007" * 2) + "\u2009\u2009\u2009\u2009" + direita)
...
def montar_resultado_surfe_50(analise):
    """Monta o cabeçalho, as 50 rodadas e o resumo do SURF."""
...
def surfe_ver_callback(call):
    try:
        ...
        # Três mensagens: identificação, SURF conectado em duas colunas e estatística.
        ...
        mensagem_rodadas = partes2[0].strip()
        mensagem_estatistica = partes2[1].strip() if len(partes2) == 2 else ""

        for texto_mensagem in (mensagem_cabecalho, mensagem_rodadas, mensagem_estatistica):
            if not texto_mensagem:
                continue
            for pos in range(0, len(texto_mensagem), 3900):
                trecho = texto_mensagem[pos:pos + 3900]
                if texto_mensagem == mensagem_rodadas:
                    m = bot.send_message(chat_id, trecho)
                else:
                    m = bot.send_message(chat_id, trecho)
                surfe_mensagens_abertas.setdefault(chat_id, []).append(m.message_id)
