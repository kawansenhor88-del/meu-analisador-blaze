...
def _montar_surfe_duas_colunas(registros):
    """Monta as duas colunas do SURF mantendo a primeira intacta e alinhando a segunda."""
    ...
    def largura_visual(texto):
        """Estimativa de largura para compensar a fonte proporcional do Telegram."""
        largura = 0.0
    ...
    largura_alvo = max((largura_visual(linha) for linha in vermelho), default=0.0) + 1.5
    ...
    for esquerda, direita in zip(vermelho, preto):
        faltando = max(0.0, largura_alvo - largura_visual(esquerda))
        inteiros = int(faltando)
        resto = faltando - inteiros

        preenchimento = "\u2007" * inteiros
        if resto >= 0.75:
            preenchimento += "\u2007"
        elif resto >= 0.35:
            preenchimento += "\u2006"
