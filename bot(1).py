...
# O banco guarda TODO o histórico. O deque acima continua sendo apenas
# um cache rápido das últimas 200 rodadas em memória.
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            """, (
                ...
                None if rodada.get("numero") is None else str(rodada.get("numero")),
                rodada.get("instant"),
...
def adicionar_rodada(payload):
    ...
    # Primeiro grava no banco permanente. Se o banco falhar, não avançamos
    # o histórico em memória para evitar a falsa impressão de persistência.
    if not salvar_rodada_banco(rodada_id, rodada):
        print("RODADA NÃO FOI GRAVADA NO BANCO. Ela não será adicionada ao cache.")
        return False
...
def obter_historico():
    # O banco é a fonte permanente. O deque continua como cache local.
